...
LocalFileStorage.prototype.upload = function(file, context, next) {
	...
	fs.mkdir(basedir, function() {
		...
		if(self.encryptionEnabled && self.encryptionKey) {
			...
			var cipher = crypto.createCipheriv('aes-256-ctr', self.encryptionKey, iv);
		...
		if(token) {
			...
			fs.readFile(bundlePath, 'utf8', function(err, data) {
				try {
                    ...
                    var iv = Buffer.from('26ae5cc854e36b6bdfca366848dea6bb', 'hex');
										var decipher = crypto.createDecipheriv('aes-256-ctr', self.encryptionKey, iv);
		...
		if(token) {
			...
			fs.readFile(tokenPath, 'utf8', function(err, data) {
				try {
					...
					var mimetype = mime.getType(file) || context.type;
