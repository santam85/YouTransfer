...
Router.prototype.download = function() {
	return function(req, res, next) {
		...
		req.errors.register([
			...
			{
				code: 'ENOTFOUND',
				message: 'Oh no... you did not specify a token! Please try again!',
				html: 'Oh no... you did not specify a token!<br />Please try again!'
			},
			{
		...
		if(token) {
			if(validator.isUUID(token.toString())) {
				youtransfer.archive(token, res, next);
			} else {
...
Router.prototype.staticFiles = function() {
	return restify.plugins.serveStatic({
		directory: './dist',
		maxAge: nconf.get('CACHE_MAX_AGE')
	});
};
